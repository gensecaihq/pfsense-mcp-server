...
- **Forward-compat CI probe for MCP spec 2026-07-28.** A non-blocking CI job runs the full suite against fastmcp 4.0.0b2 (mcp SDK 2.x, sessionless protocol). The suite passes unmodified, so adopting the stateless protocol when fastmcp 4 leaves beta is a pin change; fastmcp 4 negotiates the protocol era per connection, preserving backward compatibility with handshake-era clients.
