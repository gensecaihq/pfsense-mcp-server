# Changelog

All notable changes to the pfSense MCP Server will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [Unreleased]

## [1.1.0] - 2026-09-23

Wire-format correctness ("wire truth"), safety hardening, and client
robustness since 1.0.0. Tool count went from 327 to 332: new ACME/HAProxy
sub-resource tools and `get_log_file` were added, and two tools that could never
work against the REST API were removed (see **Removed**). The test suite grew
from 308 to 661.

### Upgrade notes

Most users need no changes. Check these if they apply to you:

- **Boolean env vars are parsed strictly.** `VERIFY_SSL`, `MCP_READ_ONLY`,
  `MCP_ENABLE_LOG_FILES` and `ENABLE_HATEOAS` accept `true/false`, `1/0`,
  `yes/no`, `on/off`; any other value is now a startup error instead of a
  silent guess.
- **`docker-compose.yml` publishes the HTTP port on `127.0.0.1` only.** Set
  `MCP_BIND_ADDR=0.0.0.0` to expose it on the network (behind TLS).
- **Settings changes are rate-limited** (`update_*`, `apply_*`, `enable_*`,
  `disable_*`, `control_service`): 30 per 60s by default, tunable with
  `MCP_RATE_LIMIT_UPDATE`.
- **`update_system_dns(dnslocalhost=...)` takes `"local"` or `"remote"`**, not
  a bool. The bool form never worked; it was rejected by the API.
- **`restore_config_backup` and `get_dns_forwarder_settings` are gone**. They
  called endpoints that do not exist. Guarded changes still capture a rollback
  revision; restore it from the webGUI (Diagnostics > Backup & Restore >
  Config History).
- **Install from Git, not PyPI.** The PyPI package named `pfsense-mcp-server`
  belongs to an unrelated project.

### Added

- **`get_log_file`: raw reads of the main log files, including `resolver.log`** (PR #90 by @cbrown350). The REST API has no resolver-log endpoint. Its log endpoints also load the whole file into PHP before applying the limit, which can run pfSense out of memory (pfSense-pkg-RESTAPI#806). `get_log_file(log_file, lines=100, grep=None)` reads the newest N lines of an allowlisted `/var/log` file (`dhcpd`, `filter`, `resolver`, `system`, `auth`) through the command-prompt endpoint, and `tail` caps the output on the pfSense side. The optional fixed-string `grep` runs *before* `tail`, so you get the newest N *matching* lines. The pipeline is grouped under `set -o pipefail`, so a failed read returns an error instead of an empty success. The tool is on by default; `MCP_ENABLE_LOG_FILES=false` removes it at startup. The OOM workaround hint no longer suggests `clog`, which no supported pfSense ships.
- **Opt-in GCF response encoding** (PR #87 by @blackwell-systems). With `RESPONSE_FORMAT=gcf` and the optional extra installed (`pip install '.[gcf]'`), each tool result comes back as a single [Graph Compact Format](https://gcformat.com) block that states repeated record field names once. That cuts tokens on large read results: 53% on a 30-rule firewall listing. GCF is used only when it is smaller *and* round-trips losslessly, and `structuredContent` is preserved. Without the extra, the server logs a warning and stays on JSON, which remains the default.
- **`MCP_RATE_LIMIT_UPDATE`**: rate limit for LOW-risk settings changes (see **Security**).
- **`PFSENSE_CA_FILE` — keep TLS verification on with a private CA** (issue #73 by @pbhorjee, PR #83). pfSense presents a certificate from its own CA, and Python does not read the OS trust store, so `VERIFY_SSL=true` failed the handshake and the documented way out was to disable verification — putting the API credential on a connection nobody authenticated. The setting points at that CA's PEM; the client resolves httpx's `verify` argument once at construction to a bool or an `ssl.SSLContext` from `create_default_context`, with `CERT_REQUIRED` and hostname checking left at their secure defaults (asserted in tests — a context that quietly relaxed either would verify nothing while looking correct). A missing, non-file, or unparseable bundle raises at startup rather than degrading into an unverified connection, and the error names only the path and exception type, never file contents. `VERIFY_SSL=false` remains the explicit opt-out and still wins when both are set, but now warns that the bundle is ignored.

- **`SECURITY.md`** — private vulnerability-disclosure policy and deployment-hardening guidance for a firewall-management tool.
- **Wire-contract test layer.** `scripts/generate_contract.py` distills the
  upstream pfSense REST API OpenAPI spec (a pkg-RESTAPI release asset) into a
  slim, vendored contract (`tests/contract/contract-v2.9.0.json`, since bumped to v2.10.2) of every
  writable endpoint's valid fields, JSON types, enum choices, and
  create-required fields. `tests/contract/schema.py` asserts a tool's actual
  wire payload against it. This catches the class of bug where pfSense silently
  drops an unknown field (PATCH) or 400s on a mistyped/missing one (POST) —
  failures invisible to a mocked unit test. Regenerate with
  `python scripts/generate_contract.py <openapi.json> tests/contract/contract-<ver>.json --version <ver>`.
- **Schedule-based firewall rules.** The schedule tools (`create_firewall_schedule`, `create_schedule_time_range`, etc.) could build schedules, but no rule tool could reference one, so schedules were inert. `create_firewall_rule_advanced` and `update_firewall_rule` now expose a `schedule` parameter (sent as the API's `sched` field) to assign an existing schedule to a rule; on update, passing `""` detaches the schedule.
- **IPv6 / dual-stack firewall rules** (PR #10). `create_firewall_rule_advanced` exposes an `ipprotocol` parameter (`inet`, `inet6`, `inet46`) with validation instead of hardcoding `inet`.
- **uvx / pipx installation** (#8). Added a `pfsense-mcp-server` console entry point and setuptools package discovery, so the server can run without cloning the repository (`uvx --from git+https://github.com/gensecaihq/pfsense-mcp-server pfsense-mcp-server`; the project is not on PyPI).
- **End-to-end MCP protocol smoke test** (`scripts/inspector_smoke.sh`, `make test-e2e`, CI job `mcp-inspector-e2e`). Drives the server over the real MCP wire protocol with the official [MCP Inspector](https://github.com/modelcontextprotocol/inspector) CLI on both transports: initialize handshake, full tool listing with annotations and descriptions, guardrail confirm-gate on a destructive call, `MCP_READ_ONLY` tool reduction, unauthenticated `/health`, 401-without-bearer and 403-bad-Origin enforcement, and an authenticated HTTP session. Runs in CI on every push with no pfSense instance (TEST-NET black-hole target), so transport and handshake regressions fail CI rather than surfacing in a user's Claude Desktop.

### Removed

- **`restore_config_backup`**. It sent `PATCH /diagnostics/config_history/revision`, but that endpoint only supports GET and DELETE, and the REST API has no config-restore endpoint at all. It always failed, yet every guarded destructive result told the user to call it to undo. The rollback instruction now points at the webGUI's Config History page with the captured revision ID.
- **`get_dns_forwarder_settings`**. It read `/services/dns_forwarder/settings`, which does not exist in any supported pkg-RESTAPI version (404). The DNS Forwarder host-override tools are unaffected.

### Fixed

- **`search_dhcp_static_mappings` always reported no reservations** (PR #91 by @JeremiahChurch). It queried `/services/dhcp_server/static_mappings`, which does not exist on pkg v2.7.3, and swallowed the 404 as "zero results", so real reservations looked absent on every interface. The tool now reads each server's embedded `staticmap` from `/services/dhcp_servers` and filters, sorts and pages client-side. An unknown interface now returns an error instead of an empty result. The `interface` default changed from `"lan"` to all servers, so MAC/IP lookups no longer skip other VLANs. `_lookup_mapping_parent_id` now raises when a bare mapping ID exists on several interfaces, instead of taking the first hit, which could have aimed a PATCH/DELETE at the wrong interface.
- **Six tools were rejected by the REST API on every call that set the affected field** (found by the v1.1.0 release audit against the v2.10.2 contract; each now has a wire-contract test):
  - `update_ssh_settings` and `update_email_notification_settings` sent `port` as an int; upstream models it as a string.
  - `update_system_dns` sent `dnslocalhost` as a bool; upstream is an enum, `local` or `remote`.
  - `create_traffic_limiter` / `update_traffic_limiter` sent a scalar `bandwidth` plus a nonexistent `bandwidthtype` and `descr`, and omitted the create-required `sched` and `aqm`. They now send `bandwidth` as `[{bw, bwscale}]` (Gb is converted to Mb, since upstream accepts only b/Kb/Mb) and `description`. They also expose `sched` (default `wf2q+`) and `aqm` (default `droptail`).
  - `search_bind_zone_records` queried `/services/bind/zone/records`, which does not exist. It now reads the zone's embedded `records` and filters, sorts and pages client-side.
- **A non-ASCII bearer token crashed the HTTP auth middleware with a 500.** `hmac.compare_digest` raises `TypeError` on non-ASCII `str`; tokens are now compared as bytes (a clean 401), and the `Origin` header is decoded as latin-1 so a malformed value can't raise either.
- **The Docker image had no version label.** The OCI `LABEL`s were set in the builder stage, so they were dropped from the final image. They now live in the final stage, together with `org.opencontainers.image.source`.
- **A write could be applied twice after an HTTP 503** (PR #74 by @pbhorjee). pfSense can commit a change and *then* return 503 while the `apply` restart is in flight, so the response carries no information about whether it landed. 503 was retryable for every method, so a POST/PATCH/DELETE could reapply the same change — the exact failure the retry policy existed to prevent. 503 now joins 502/504 as idempotent-GET-only; 429 stays retryable for all methods, since there the server is stating it did *not* process the request.
- **Concurrent alias edits could silently undo each other** (PR #76 by @pbhorjee). `remove_from_alias` reads the address/detail lists and PATCHes a rebuilt copy, so two concurrent removals could read the same snapshot and the later write restore an address the earlier one deleted. Alias mutations now serialize on a process-local per-alias lock held across the whole GET→filter→PATCH. The locks are cleared whenever the event loop changes (`reset()` and the loop-change branch in `_ensure_client`): `asyncio.Lock` records its loop only when an acquire actually blocks, so a *contended* lock stays latched to a dead loop and raises "bound to a different event loop" on reuse. This coordinates one MCP process only — it does not replace pfSense-side or cross-process locking.
- **Retries could stack unbounded delay and fire in lockstep** (PR #77 by @pbhorjee). Exponential backoff now carries ±25% jitter (applied inside the cap, so `_BACKOFF_CAP` stays a real ceiling), and all waits for one request — including `Retry-After` — draw from a single 10s budget, after which the error surfaces instead of another sleep.
- **Transport errors lost their request context** (PR #75 by @pbhorjee). `_describe_transport_error` rebuilds httpx exceptions to attach a readable message, but dropped the attached `Request`, so callers inspecting `exc.request` got nothing or a `RuntimeError`. The request is now carried through, with fallbacks for exceptions that have none and for constructors that reject the keyword.
- **`update_interface` and `delete_interface` could not address any interface** (PR #78 by @DrewKolstad, found during a management-VLAN renumber on a Netgate 8200). Both declared `interface_id: int`, but the API keys interfaces by config name (`opt16`, `wan`, `lan`), so every valid call failed Pydantic validation before a request was sent, and a numeric id would have targeted the wrong object. Now `str` — deliberately not `Union[int, str]`, so a wrong call fails before it reaches the firewall. The vendored contract cannot catch this class of bug: upstream declares `id: integer` on all 88 PATCH endpoints, which is why `tests/contract/schema.py` exempts `id`/`parent_id` from type checking, so all 81 `*_id: int` tools were audited by hand against config structure. Interfaces were the only name-keyed case.
- **Client-side search filters crashed on null fields, and matched "none" against every null row** (PR #78 by @DrewKolstad). `i.get("ipaddr", "").lower()` returns `None` when the key is present but null, so one null row failed a whole page — reproducible on any box with `typev4: "none"` interfaces or a lease with no client name. The `str(...)` variants had a quieter bug: `str(None) == "None"`, so searching "none" matched every row whose field was null. 93 sites across 20 modules now share `helpers.field_contains`, which skips nulls.
- **`find_object_by_field` could not reach any collection endpoint** (PR #78 by @DrewKolstad). The allowlist held prefixes ending in `/`, so `/interfaces` never matched `/interface/` — defeating the tool for exactly the name-keyed objects that most need a stable reference. Matching is now on the whole first path segment, singular or plural, which also rejects `/systemfoo` (it satisfied the old `startswith` check). Follow-up in PR #82: the plural check used `rstrip("s")`, which strips *every* trailing `s` and so admitted `/systemss`; it now strips at most one.
- **`get_pf_table` and `get_config_revision` had never worked** (PR #80 by @msarg44). Both passed `params=` to `crud_get_settings`, which took only `endpoint` — a `TypeError` on every invocation. The helper now forwards to `extra_params`. Nothing had failed because no test exercised it with params, so the call sites and the signature drifted apart unnoticed; PR #82 adds that coverage, and widens the query-value type to `Union[str, int]` to match callers like `get_config_revision`, which passes an int revision id. Separately, `get_pf_table` was also querying `?name=` when the endpoint keys by name-as-id; PR #78 corrected it to `?id=`.
- **WireGuard peer allowed IPs sent CIDR in the address field** (PR #78 by @DrewKolstad). The endpoint types `address` as a bare IP with `mask` as a separate required integer, so CIDR notation is now split into the pair, with a bare address becoming a host route (/32 or /128) and a conflicting explicit `mask` rejected.
- **`create_dhcp_static_mapping` could not set per-host lease times** (PR #78 by @DrewKolstad). `defaultleasetime`/`maxleasetime` now pass through, including zero. The package materializes 7200/86400 whenever the field is absent, and a later PATCH sending an explicit null returns 200 but reads back unchanged — so omitting them is not neutral, which the docstring now states. Whether the DHCP daemon acts on the per-host values is recorded as unverified.
- **`get_pf_table` echoed an unstripped name** (PR #82). `table.get("name", name)` falls back only when the key is *absent*, but the API returns an explicit null and the request went out with `name.strip()`, so a padded argument came back padded.

- **`API_TIMEOUT` was silently ignored — every API request ran with timeouts
  disabled.** The retry refactor passed `timeout=None` to httpx for any request
  without a per-request override, and httpx treats an explicit `None` as
  "disable timeouts", not "use the client default". Against an unreachable
  pfSense, each attempt hung until the OS abandoned the TCP connect (~75s), so
  a single tool call could stall for minutes. Requests now pass
  `httpx.USE_CLIENT_DEFAULT`, restoring the configured `API_TIMEOUT`
  (regression-tested). Found by driving the server with the MCP Inspector CLI
  against a black-holed TEST-NET address.
- **An unreachable pfSense delayed MCP startup past client timeouts.** The
  preflight connectivity check runs before the MCP transport opens and used the
  full retry pipeline (up to ~4× `API_TIMEOUT` plus backoff) — with the timeout
  bug above, several minutes. Claude Desktop and the MCP Inspector mark a server
  dead long before that. The preflight is now hard-bounded to 5 seconds
  (`asyncio.wait_for`); on expiry the server logs a warning and starts anyway,
  per the existing degraded-start design (PR #14).
- **Connectivity failures surfaced as an empty error string.** `str()` of
  httpx timeout exceptions is often empty, so tools reported
  `{"success": false, "error": ""}` — useless to an operator. Transport errors
  are now re-raised with a descriptive message (`Cannot reach pfSense at <host>:
  ConnectTimeout (timeout 30s, 4 attempt(s))`) with the exception type
  preserved for existing handlers.
- **Firewall log searches could return stale or wrong-IP results** (PR #23 by @pbhorjee). On pfSense 2.8.1 the server-side `text__contains` filter can return an old, non-chronological log slice (pfSense-pkg-RESTAPI #806/#860), making current firewall audits appear stale or empty; and IP searches matched raw text, so `10.0.0.1` also matched `10.0.0.10`. `get_firewall_log`, blocked-traffic analysis, and firewall IP searches now fetch the newest log window and filter locally in the shared client helpers — with exact matching on parsed `src_ip`/`dst_ip` — so the `troubleshoot.py` diagnostics benefit automatically. The search covers the newest ≤50 raw lines (the PHP-memory-safety cap), which the docstrings state explicitly.
- **Live health diagnostics reported wrong data.** `diagnose_dns_resolution`/`diagnose_service_health`/`get_system_health_report`/`diagnose_connectivity`/`diagnose_interface_issues` read runtime gateway status from the `/routing/gateways` *config* endpoint (which has no `status` field), so every gateway was classified `unknown` and `diagnose_connectivity` appended a false "Gateway X is unknown" issue on every run; DNS servers were read from the system-status payload instead of `/system/dns`; and service status wasn't normalized for the API v2 boolean form. Now sourced from `/status/gateways` and `/system/dns` with boolean-status handling. (Original fix by @pbhorjee in #21, completed with the two remaining gateway call sites and a test suite.)
- **A momentary network blip or `503`/`429` failed the whole tool call.** The API client now retries transient failures with exponential backoff (capped): connection errors and `429`/`503` are retried for any method (a `Retry-After` header is honored, bounded); read-timeouts and `502`/`504` are retried only for idempotent `GET`s. Non-idempotent writes (POST/PATCH/DELETE) are never retried on an ambiguous read-timeout or gateway error, so a change can't be silently applied twice, and the fast-fail log endpoints (which set a short read timeout) opt out entirely. Also bounded the httpx connection pool (`max_connections=10`) so a burst of tool calls can't overwhelm pfSense's PHP-FPM workers.
- **The input sanitizer rejected legitimate values** (e.g. multi-container LDAP DNs like `CN=Users;DC=example,DC=com`, OpenVPN custom options, pipes in descriptions). Its shell-injection rules (`;\s*\w`, `|\s*\w`, backtick, `$(`, `${`) were security theater for this codebase — tool values are sent to the pfSense API as JSON *data*, never into a shell (the one real command sink is hard-allowlisted separately) — while false-positiving on real config. The denylist is now scoped to threats meaningful for stored data: path traversal and a `<script>` tag; field-level positive validation (IP/port/MAC/…) does the real input checking.
- **A failed rollback-point capture was silently swallowed.** For HIGH/CRITICAL operations the `@guarded` gate captures the pre-change config revision so the change can be undone, but a capture failure was `except: pass` — the operation proceeded with no rollback pointer and no warning, contradicting the "always backs up" docs. The tool result now carries a `config_backup_warning` when no rollback point could be captured.
- **`export_*` tools bypassed write-safety.** `export_certificate_pkcs12` (a private-key bundle) and `export_openvpn_client_config` were classified READ via the `export_` prefix and annotated `readOnlyHint=True`, so they were retained in `MCP_READ_ONLY` mode and skipped rate-limiting/audit despite issuing POSTs that extract secrets. They are now classified MEDIUM, `@rate_limited`, and annotated `readOnlyHint=False`.
- **The Docker HTTP-mode health check could never pass.** The Dockerfile and compose health checks probed `/mcp` unauthenticated, but the bearer-auth middleware 401s every request without a token — so a `streamable-http` container reported unhealthy forever. The middleware now answers an unauthenticated, secret-free `/health` (200), and the health checks point at it; `/mcp` still requires a valid bearer token.
- **Secrets could leak through echoed API error bodies.** On a 4xx, pfSense echoes the offending field values (which can include a submitted `password`/`pre_shared_key`/`ldap_bindpw`/`radius_secret`/…), and the full body was surfaced in the tool's error and logs. Error bodies are now run through the secret redactor before display, and a non-JSON error body is withheld entirely. The redactor's field list was widened from exact-match to include secret-indicating substrings, so provider-specific fields (`radius_secret`, `ldap_bindpw`, `ipsecpsk`, `authorizedkeys`, `webrootftppassword`, `cpanel_apitoken`, …) are caught without over-redacting public fields like `publickey`/`keylen`.
- **HTTP transport could boot with a publicly-known bearer token.** Startup previously rejected only an *empty* `MCP_API_KEY`, so a deployment left on the documented `CHANGE-ME` placeholder would run with a guessable token. It now also rejects placeholder values and tokens shorter than 16 characters (per key, for the comma-separated multi-key form).
- **33 mutating tools shipped with no guardrails at all** — no rate limiting, input sanitization, allowlist check, or audit trail. These included every `apply_*_changes` tool and `control_service` (which can stop `sshd`/`dhcpd`/`unbound`). All 33 now carry `@rate_limited`. A new registration-time meta-test (`tests/test_guardrail_coverage.py`) fails if any non-READ tool is undecorated or any HIGH/CRITICAL tool lacks the full `@guarded` confirm gate, so a forgotten decorator can no longer ship a silently-ungated tool. (The `@guarded`/`@rate_limited` decorators now carry a `_guardrail` marker for this check.)
- **Search tools missed matches beyond the first page.** 45 `search_*` tools filter their `search_term` client-side, but did so *after* server-side pagination — so a match on page 2+ was silently invisible and the returned `count` was wrong. A new `create_search_pagination` helper fetches the full window (up to the 200-object `MAX_PAGE_SIZE` cap) whenever a search term is active, so the filter sees every object; non-search listing is unchanged. (Configs larger than 200 objects of one type still only search the top 200 — the API's memory-safety cap.)
- **IPsec Phase 2 encryption entries could not be created.** The `IPsecPhase2Encryption` model has only `name` and `keylen`, but `create`/`update_ipsec_phase2_encryption` sent `encryption_algorithm_name`/`encryption_algorithm_keylen` plus phase1-only fields (`hash_algorithm`/`dhgroup`/`prf_algorithm`), so create 400'd on the missing required `name` and the extras were silently dropped. Now sends `name`/`keylen`; the phase1-only params are accepted for compatibility but no longer sent, and the search default sort moved to `name`. (The Phase 1/Phase 2 *create* tools require nested `encryption`/`hash_algorithm_option` proposal models that still need live validation before rebuild — tracked as a follow-up.) Verified against the pkg-RESTAPI v2.9.0 contract.
- **The OpenVPN tool family was non-functional and, worse, unsafe.** `create`/`update_openvpn_server` and `_client` sent `crypto` (upstream `data_ciphers`, an array), `ca`/`cert` (→`caref`/`certref`), `descr` (→`description`), `disabled` (→`disable`), and int `local_port`/`server_port`/`dh_length`/`proxy_port` where the upstream PortField/StringField want strings — so servers and clients could not be created. Two of these were security-relevant: a dropped `disabled` created a **live** VPN listener when the caller asked for a disabled one, and `manage_openvpn_cso` sent `server_id` (upstream `server_list`, an array of interface names) which, when dropped, silently applied the client-specific override to **all** servers. `compression` is mapped to `allow_compression` (`no`/`yes`/`asym`), `custom_options`/`local_network`/`remote_network` are sent as arrays, `redirect_gateway`→`gwredir`, and every OpenVPN search filtered/sorted on `descr` (a nonexistent field that matched everything) — now `description`. `export_openvpn_client_config` used all-wrong field names (`server`/`type`/`usetoken`/`silent` upstream); the mappable fields are corrected (some export knobs still need live validation). Verified field-by-field against the pkg-RESTAPI v2.9.0 contract.
- **WireGuard peers and tunnels were misconfigured or rejected.** Peers sent `keepalive` (upstream field is `persistentkeepalive`), referenced their tunnel by array-index `id` instead of by name (`tun` is a name reference upstream), and sent `port`/`listenport` as integers where the upstream PortField is string-typed. New peers were also created without `enabled`, which defaults to *false* upstream — so an MCP-created peer was inert until separately enabled. `create`/`update_wireguard_peer` and `create`/`update_wireguard_tunnel` now send `persistentkeepalive`, tunnel names, string ports, and an explicit `enabled` (default True). Verified against the pkg-RESTAPI v2.9.0 contract.
- **User groups and LDAP auth servers could not be configured.** Groups sent `descr` (upstream field is `description`) and `member` as array-index user IDs (upstream references users by **name**), so descriptions and membership never took. LDAP auth-server tools sent generic `port`/`transport`/`scope`/`basedn`/`authcn` with a `tcp`/`ssl`/`starttls` transport vocabulary; upstream uses `ldap_port`/`ldap_urltype`/`ldap_scope`/`ldap_basedn`/`ldap_authcn` with url-type values `Standard TCP`/`STARTTLS Encrypt`/`SSL/TLS Encrypted`, and the port fields are string-typed — so LDAP servers could never be created. All corrected (tool parameter names unchanged); RADIUS auth-server ports are likewise coerced to strings.
- **Certificate & CA import, renewal, and PKCS#12 export were all non-functional.** Import sent `cert` (upstream field is `crt`) plus an unknown `method` key, so `create_certificate`/`create_certificate_authority` 400'd; internal generation was POSTed to the *import* endpoint, where its key/DN fields were silently dropped. `create_certificate`/`create_certificate_authority` now route `method="import"` to the import endpoint (`crt`/`prv`) and `method="internal"` to `/generate` (with a new `ecname` parameter, required for ECDSA); `update_certificate`/`update_certificate_authority` send `crt`; `generate_certificate` drops the bogus `method` and sends `ecname` for ECDSA keys. `renew_certificate` and `export_certificate_pkcs12` sent the non-persistent array index as `id`, but both endpoints key on `certref` (the certificate's stable refid) — they now resolve the refid first. Verified field-by-field against the pkg-RESTAPI v2.9.0 contract.
- **Boolean search filters returned inverted results.** `QueryFilter` serialized Python bools with `str()`, yielding `"True"`/`"False"`; the pfSense query engine only coerces the lowercase `true`/`false` to booleans and loose-compares everything else as truthy. So `search_firewall_rules(disabled=False)` — "show enabled rules" — returned exactly the *disabled* rules. `QueryFilter.to_param` now lowercases booleans, fixing the whole class at the root.
- **`search_services(status_filter="stopped")` returned the running services.** `Service.status` is a boolean upstream, so the string `"stopped"` loose-matched everything; `find_running_services`/`find_stopped_services` now filter on real booleans (`True`/`False`).
- **`MCP_READ_ONLY=true` crashed the server at startup** (regression from the FastMCP 3.4.6 upgrade). The read-only tool reduction reached into FastMCP 2's private `mcp._tool_manager._tools`, which FastMCP 3 removed, so the documented least-privilege mode raised `AttributeError` and would not start. It now uses FastMCP's public `mcp.local_provider` API (`list_tools` / `remove_tool`), producing the same read-only tool set (131 tools at the time). The reduction also moved out of module-import scope into `main()` (as `apply_read_only_filter()`): doing async work while the module was still importing could deadlock against Python 3.11's import lock. Added `tests/test_read_only_mode.py`, which runs the filter in isolated subprocesses (hard timeout, hermetic env) so neither the crash nor the hang can regress silently.
- **`src.__version__` reported `5.0.0`** while every other version string (`pyproject.toml`, `server.py`, `Dockerfile`) said `1.0.0`. `src/__init__.py` now derives `__version__` from `server.VERSION`, single-sourcing the two.
- **DNS Resolver DHCP registration was a silent no-op.** `update_dns_resolver_settings` mapped `register_dhcp`/`register_dhcp_static` to themselves, but the upstream fields are `regdhcp`/`regdhcpstatic`; PATCH silently dropped the unknown keys and reported success. Now mapped correctly (tool parameter names unchanged).
- **DHCP DNS-server override always failed.** `create_dhcp_static_mapping` and `update_dhcp_server_config` sent `dnsserver` as a bare string, but upstream it is an array of up to 4 strings. The tools now accept one value or a comma/space-separated list, validate each as an IP, and send an array.
- **DNS Resolver access-list tools could not create or update.** They sent `aclname`/`aclaction`/`descr` (internal-only names) with underscore action values; upstream uses `name`/`action`/`description` with space-separated actions (`allow snoop`, `deny nonlocal`, `refuse nonlocal`). Create 400'd; update silently no-op'd. The tools keep their ergonomic parameter names/spellings and map to the wire values; `search_dns_access_lists` default sort moved from `aclname` to `name`.
- **`remove_from_alias` failed on aliases with per-entry descriptions** (400 `TOO_MANY_ALIAS_DETAILS`). The API's remove control flag strips only `address` entries, leaving the parallel `detail` list longer than the address list, which the API then rejects. `manage_alias_addresses(action="remove")` now reads the alias and rebuilds both lists in lockstep before PATCHing.
- **`create_firewall_schedule` could not actually create a schedule.** The pfSense API requires at least one time range at creation ("Field `timerange` is required"), but the tool never sent one. It now takes `hour`/`position`/`month`/`day`/`rangedescr` for the initial time range (further ranges via `create_schedule_time_range`) and validates that either `position` (weekdays) or `month`+`day` is provided. Also corrected `month`/`day`/`position` on `create_schedule_time_range`/`update_schedule_time_range` from `str` to `List[int]` to match the API schema, with docstrings explaining pfSense's semantics (`position` = weekday numbers 1–7). Verified end-to-end against a live pfSense 26.03 instance.
- **`create_acme_certificate` could not actually create a certificate.** The pfSense API rejects certificates with no domain/SAN entries ("Field `a_domainlist` is required"), but the tool never exposed that field. Added a required `a_domainlist` parameter (and an optional one on `update_acme_certificate` to replace the whole list); added `search_acme_certificate_domains` and `manage_acme_certificate_domain` to read and incrementally add/remove individual domain entries (DNS-01 via a provider API, HTTP-01, webroot, etc.) without resending the whole array.
- **`issue_acme_certificate`/`renew_acme_certificate` always failed** ("Field `certificate` is required"). Both endpoints key off the certificate's `name`, not the array-index `id` the tools took as input. They now resolve `id` → `name` internally and send the correct field, with no change to the tools' own `id` parameter.
- **ACME issuance could hang forever with no error on networks that block DNS-over-HTTPS.** Without `dnssleep`, acme.sh polls public DoH resolvers (`cloudflare-dns.com`, `dns.google`, etc.) to self-check DNS-01 propagation before contacting Let's Encrypt; if those are blocked (a common DNS-hardening setup), the check loops every 10 seconds indefinitely and the REST API reports `status: pending` forever with an empty log. Reproduced live against a real pfSense instance — confirmed via SSH that the underlying `acme.sh` process was alive but stuck in that retry loop. Added `dnssleep` to `create_acme_certificate`/`update_acme_certificate` to skip the check and just wait a fixed number of seconds instead.
- **`create_haproxy_frontend`/`update_haproxy_frontend` silently dropped bind addresses.** They sent a `bind_addresses` field that doesn't exist in the pfSense API; the real field is `a_extaddr`. Fixed the outgoing field name (the tool's own `bind_addresses` parameter is unchanged) and added `ssloffloadcert`/`ha_certificates` so a frontend's SSL certificate can actually be set — previously there was no way to bind a cert to a frontend at all. Added `search_haproxy_frontend_addresses`/`manage_haproxy_frontend_address` and `search_haproxy_frontend_certificates`/`manage_haproxy_frontend_certificate` for incremental add/remove of individual bind addresses and SNI certificates.
- **All `delete_*` tools were non-functional** (#12, PR #9, PR #16). `httpx.AsyncClient.delete()` does not accept a `json=` kwarg, so every delete (firewall rules, NAT, aliases, DHCP mappings, etc.) raised `TypeError` before any HTTP traffic. DELETE now routes through `client.request("DELETE", ...)`, which supports the JSON body pfSense requires.
- **`update_log_settings` could not enable remote syslog and silently dropped fields** (#13, PR #11). The wire-format keys `ipproto` and `reverse` were ignored by the API; renamed to `ipprotocol` and `reverseorder`. Added `enableremotelogging` (the master toggle), `logconfigchanges`, and the per-category remote-syslog toggles (`auth`, `portalauth`, `vpn`, `dpinger`, `hostapd`, `system`, `resolver`, `ppp`, `routing`, `ntpd`).
- **`update_webgui_settings` could not change the WebGUI port** (#7). The pfSense REST API requires `port` as a string; the tool now accepts an `int` for ergonomics and coerces it to a string before sending.
- **Log endpoints could hang until pfSense ran out of memory** (PR #6). Added a 10-second read-phase timeout and classification of read-phase failures (`ReadError`/`RemoteProtocolError`/`ReadTimeout`) into a clear, actionable message (upstream tracking: pfSense-pkg-RESTAPI#806), with docstring warnings on the log tools.
- **Transient connectivity blip at launch killed the server** (PR #14). A momentary preflight failure before the stdio channel opens now logs a warning and starts anyway; individual tools surface connectivity errors when invoked.

### Security

- **`VERIFY_SSL=1` silently disabled TLS verification.** Only the exact string `true` counted, so `1`, `yes`, `on`, or a stray space turned certificate checking *off*, and the startup warning (which fired only on the exact string `false`) stayed quiet. `MCP_READ_ONLY=1` likewise left every write tool registered. All boolean env vars now go through one strict parser (`helpers.env_bool`) that accepts the usual spellings and refuses anything else at startup.
- **Settings changes were not rate-limited.** The 79 LOW-risk tools (`update_*`, `apply_*`, `enable_*`, `disable_*`, `control_service`) carried `@rate_limited`, but `check_rate_limit` only had buckets for MEDIUM and above, so a runaway loop could PATCH the webGUI port, SSH, or the WAN interface without limit. LOW now has its own bucket (`MCP_RATE_LIMIT_UPDATE`, default 30 per 60s), and a test checks the behavior, not just the decorator marker.
- **`docker-compose.yml` exposed the plain-HTTP MCP port on every host interface**, which sent the bearer token in cleartext across the LAN. The port now binds `127.0.0.1` by default (`MCP_BIND_ADDR` overrides it). Compose also passes `PFSENSE_CA_FILE` through and mounts `./certs` read-only, so a private-CA pfSense no longer pushes Docker users toward `VERIFY_SSL=false`.
- **Install instructions pointed at an unrelated PyPI package.** The GCF docs and the server's missing-extra warning said `pip install 'pfsense-mcp-server[gcf]'`, but that PyPI name belongs to a different project, so following them installs third-party code. All five references now install from the repo or name `gcf-python` directly, and the README and `pyproject.toml` warn against installing by name.

- **An `X-API-Key` could be forwarded across a redirect** (PR #72 by @pbhorjee). httpx strips `Authorization` and `Cookie` on cross-origin redirects but treats custom auth headers as ordinary, so following one could hand the pfSense API key to another host. Redirect following is now disabled and any 3xx raises with the status and `Location` path, without parsing the body. Follow-up in PR #81: that error now names the likely cause instead of only the mechanism — `PFSENSE_URL` is not scheme-validated and pfSense redirects `http://` to `https://` by default, so an operator on `http://` sees every call fail here first. The stated reason also branches on auth method, since `X-API-Key` is only on the wire under `api_key` auth; `basic`/`jwt` send `Authorization`, which httpx *does* strip.
- **The tooling recommended disabling TLS verification** (PR #83, issue #73 by @pbhorjee). The TLS error handler advised `VERIFY_SSL=false`, both Claude Desktop examples in the README shipped it, and the installation guide's troubleshooting repeated it — so the documented path for a firewall-management tool put the API credential on an unauthenticated connection. The hint, the startup warning, both examples, the env table, and the install guide now point at `PFSENSE_CA_FILE` first, say where to export the PEM (System > Cert. Manager > CAs), and state what disabling verification actually costs.

- The secret redactor's exact-name/substring matching missed short provider credential fields like `cf_token`/`cf_key` in the new open-ended ACME `provider_fields`/`a_domainlist` dicts (DNS provider API tokens/keys — pfSense's ACME package alone has ~250 possible field names), which would have shown raw secrets in approval requests and audit logs. Added a narrow suffix check (`_key`, `_token`, `_secret`, `_password`, `_passwd`, `_pwd`) so these are redacted without over-redacting identifiers like `certificate_id` or `cf_zone_id`.

### Changed

- **fastmcp raised to `>=3.4.7`; `mcp` capped `<2.0.0`** (PR #92, supersedes #86). 3.4.7 backports the CIMD `private_key_jwt` assertion-audience security fix. The `mcp` upper bound said `<3.0.0` while the comments and Dependabot config intended 1.x, which let a grouped Dependabot bump require `mcp>=2.1`; that cannot resolve against fastmcp 3.x.
- **The fastmcp 4 CI probe now tracks the stable 4.0.x line** (`fastmcp>=4.0.5,<5`, mcp 2.2). The full suite passes on it unchanged. The release pin stays on 3.x until the migration is made deliberately.
- CI runs with a read-only `GITHUB_TOKEN` (`permissions: contents: read`).
- **Version consistency is tested.** `tests/test_version_consistency.py` fails if `pyproject.toml`, `server.VERSION` (MCP serverInfo), the Dockerfile label, and the Makefile disagree, or if CHANGELOG has no section for the version.
- CI: `actions/setup-node` 6 → 7 (PR #85).
- **pfSense CE 2.9.0 and Plus 26.07 supported** (issue #88 by @bholland-bh, PR #89). `PFSENSE_VERSION` now accepts `CE_2_9_0` and `PLUS_26_07`; all existing values keep working and the default stays `CE_2_8_1`. Both versions require REST API package v2.10.1+, which added builds for them (v2.10.2 is current); the compatibility tables in README.md and PFSENSE_API_INSTALLATION.md are updated accordingly. The vendored wire contract is bumped to `tests/contract/contract-v2.10.2.json`; the only request-schema change since v2.10.0 is additive (Kea netboot fields on DHCPServer — `netboot`, `nextserver`, `filename`/`filename32`/`filename64`/`filename32arm`/`filename64arm`, `rootpath`, `uefihttpboot`), so no tool payloads changed.
- **Wire contract updated to pkg-RESTAPI v2.10.0** (since bumped to v2.10.2 in PR #89). The vendored contract was `tests/contract/contract-v2.10.0.json`; the request schema is field-for-field identical to v2.9.0 (233 endpoints, no field/type/enum/required changes), so no tool payloads changed. Docs now recommend v2.10.0 — it fixes a command-injection flaw in the interface-group endpoints (GHSA-w3w4-mvcc-vmgr) and adds core command auto-escaping. v2.10.0 also flags `OpenVPNClient.auth_pass`, `User.ipsecpsk`, and `WireGuardPeer.presharedkey` as sensitive, so the API omits them from responses by default; this server still sets them normally (they remain in the request schema) and already redacts them in logs/errors.
- **pfSense version matrix refreshed for the current stable releases.** `PFSENSE_VERSION` now accepts `PLUS_26_03_1` (pfSense Plus 26.03.1), `PLUS_26_03`, and `PLUS_25_11_1` alongside `CE_2_8_1`; all previous values (`CE_2_8_0`, `PLUS_24_11`, `PLUS_25_11`, and the historically mislabeled `CE_26_03`, now an alias of `PLUS_26_03`) keep working. The default is now `CE_2_8_1` (current CE stable). Docs updated for REST API package v2.9.0 — the latest release, which ships builds for CE 2.8.1 and Plus 25.11.1/26.03/26.03.1 and fixes [GHSA-8q8g-9f77-8g8g](https://github.com/pfrest/pfSense-pkg-RESTAPI/security/advisories/GHSA-8q8g-9f77-8g8g) — with v2.7.3 documented as the legacy path for CE 2.8.0 and Plus 24.11/25.11. This server calls neither the settings-sync endpoint nor the new hasync endpoint, so v2.9.0's breaking changes do not affect it.
- **FastMCP upgraded from 2.14.0 to 3.4.6** (`fastmcp>=3.4.6,<4.0`). fastmcp 3 declares its dependencies correctly, so the 2.14.0-era workaround pins for `pydantic-settings` and `mcp` are replaced by a single explicit `mcp>=1.24.0,<2.0.0` (a direct dependency: the tool modules import `mcp.types.ToolAnnotations`). Server behavior is unchanged — same 327 tools, stdio + Streamable HTTP transports, bearer auth, and MCP 2025-11-25 with per-connection negotiation down to older revisions — verified end-to-end (in-memory MCP client: initialize, tools/list, tools/call; HTTP: 401 without token, 403 on bad Origin, protocol negotiation at 2025-06-18 and 2025-11-25). In fastmcp 3, `@mcp.tool` returns the plain function instead of a `FunctionTool` wrapper, so the test suite's `.fn` unwrapping was dropped; `test_enhanced_connection` (a tool whose name matches pytest's collection pattern) is now marked `__test__ = False` so pytest cannot execute the real connection tool as a test.
- **Forward-compat CI probe for MCP spec 2026-07-28.** A non-blocking CI job runs the full suite and the Inspector smoke test against fastmcp 4 (mcp SDK 2.x). It started on 4.0.0b1/b2 and now tracks the stable 4.0.x line (see above). Both pass unmodified. The smoke test exercises the handshake flow only, so sessionless (no-`initialize`) coverage is needed before the release pin moves to 4.x.
- CI is green. The `ruff check src/ tests/` step had been failing on 80 pre-existing lint issues since before the 1.0.0 tag; all are resolved with no behavior change.

## [1.0.0] - 2026-03-26

### First Stable Release

Production-ready MCP server for pfSense firewall management with 327 tools, 9-layer defense-in-depth guardrail system, and full MCP spec 2025-11-25 compliance. Verified against the pfSense REST API v2 PHP source code and validated with 568 end-to-end checks across 8 audit phases with 0 failures.

### Tools (327 total across 34 files)

- **Firewall** (43 tools) — Rules, aliases, schedules, states, virtual IPs, traffic shaping
- **NAT** (16 tools) — Port forwards, outbound NAT, 1:1 NAT
- **VPN** (51 tools) — OpenVPN, IPsec, WireGuard with full CRUD, encryption, apply, status
- **Routing** (16 tools) — Gateways, gateway groups, static routes, default gateway, apply
- **DNS** (24 tools) — DNS Resolver (Unbound) and DNS Forwarder (dnsmasq)
- **DHCP** (17 tools) — Leases, static mappings, address pools, custom options, server config
- **Certificates** (15 tools) — Certs, CAs, CRLs with generate, renew, PKCS12 export
- **Users** (12 tools) — Users, groups, auth servers
- **Interfaces** (14 tools) — Config, VLANs, bridges, groups, apply
- **System & Diagnostics** (44 tools) — Status, settings, ping, reboot, config history/restore
- **Services** (14 tools) — Core services, NTP, cron, service watchdog, SSH, Wake-on-LAN
- **Logs** (3 tools) — Firewall logs with parsed filterlog CSV (IPv4/IPv6)
- **Packages** (43 tools) — HAProxy, ACME/Let's Encrypt, BIND DNS, FreeRADIUS
- **Troubleshooting** (10 tools) — RCA diagnostics, health report, audit trail
- **Utility** (9 tools) — HATEOAS, object IDs, guardrail status, risk check

### Security — 9-Layer Defense-in-Depth

1. **Action Classification** — 5 risk levels (read/low/medium/high/critical) auto-assigned to every tool
2. **Mandatory Approval Gate** — All 52 destructive tools require `confirm=True` with full impact visualization
3. **Input Sanitization** — Recursive detection of command injection, directory traversal, XSS across all parameters
4. **Rate Limiting** — Sliding-window throttle: 20 creates/60s, 10 deletes/60s, 2 critical/300s
5. **Audit Logging** — JSON lines format with redacted parameters, pre and post execution
6. **Dry-Run Mode** — Preview any destructive operation without executing
7. **Sensitive Data Redaction** — 15 key patterns (passwords, tokens, certs) auto-redacted in all outputs
8. **Command Allowlisting** — Optional `MCP_ALLOWED_TOOLS` restriction
9. **Automatic Config Backup** — Pre-change config revision captured before every destructive operation with rollback instructions

### MCP Compliance

- MCP specification 2025-11-25 (latest)
- `serverInfo.version` and `instructions` provided
- `ToolAnnotations` on all 327 tools (readOnlyHint, destructiveHint, idempotentHint)
- Origin header validation per spec MUST requirement
- Bearer token auth with timing-safe comparison
- Default bind to 127.0.0.1 per spec SHOULD
- Read-only mode (`MCP_READ_ONLY=true`) for least-privilege deployments

### Deployment

- **Transports**: stdio (Claude Desktop/Code) and Streamable HTTP (remote)
- **Docker**: Multi-stage build, non-root user, read-only filesystem, dropped capabilities, noexec tmpfs
- **Authentication**: API Key, Basic Auth, JWT (all three pfSense REST API v2 methods)
- **Configuration**: 21 environment variables, all documented with safe defaults

### Testing

- 308 unit tests passing
- 568 end-to-end validation checks across 8 audit phases
- 0 failures in structural integrity, API contract, guardrail enforcement, rate limiting, injection attacks, real-world workflows, crash scenarios

### pfSense Compatibility

| pfSense Version | REST API Package | Status |
|---|---|---|
| pfSense CE 2.8.1 | v2.7.3 | Verified |
| pfSense Plus 25.11 | v2.7.3 | Verified |
| pfSense CE 2.8.0 | v2.6.0+ | Supported |
| pfSense Plus 24.11 | v2.6.0+ | Supported |

[Unreleased]: https://github.com/gensecaihq/pfsense-mcp-server/compare/v1.1.0...HEAD
[1.1.0]: https://github.com/gensecaihq/pfsense-mcp-server/compare/v1.0.0...v1.1.0
[1.0.0]: https://github.com/gensecaihq/pfsense-mcp-server/releases/tag/v1.0.0
