...
count change (still 327); test suite grew from 308 to 340.

### Fixed

- **`MCP_READ_ONLY=true` crashed the server at startup** (regression from the FastMCP 3.4.6 upgrade). The read-only tool reduction reached into FastMCP 2's private `mcp._tool_manager._tools`, which FastMCP 3 removed, so the documented least-privilege mode raised `AttributeError` on import and would not start. It now uses FastMCP's public `mcp.local_provider` API (`list_tools` / `remove_tool`), producing the same 130 read-only tools. Added `tests/test_read_only_mode.py`, which boots the mode in a subprocess so this can't regress silently again.
